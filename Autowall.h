...
struct FireBulletReturnDataInfo_t
{
	int m_damage;
	int m_hitgroup;
	int m_hitbox;
	...
	FireBulletReturnDataInfo_t(int damage, int hitgroup, int penetration_count, bool did_penetrate_wall, float thickness, IBasePlayer* hit_entity)
	{
		m_damage = damage;
		m_hitgroup = hitgroup;
...
public:
	...
	FireBulletReturnDataInfo_t Think(Vector pos, IBasePlayer* target, int specific_hitgroup = -1, Vector a = csgo->local->GetEyePosition());
};

extern PAutoWall g_AutoWall;