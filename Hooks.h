...
typedef void(__vectorcall* ClMoveFn)(float, bool);
typedef void(__fastcall* CLSendMoveFn)();
typedef void(__thiscall* PacketStartFn)(void*, int, int);
typedef void(__thiscall* PacketEndFn)(void*);
typedef Vector*(__thiscall* GetEyeAnglesFn)(IBasePlayer*);
#pragma endregion

//namespace get_eye_ang {
//	Vector* __fastcall hook(void* ecx, void* edx);
//	using fn = Vector * (__fastcall*)(void*, void*);
//}
...
namespace VMT
{
	...
	extern std::unique_ptr< VMTHook > g_SteamAPI;
}
using namespace VMT;
...
namespace H
{
	...
	extern GetEyeAnglesFn GetEyeAnglesO;
	extern ClMoveFn ClMove;
	extern CLSendMoveFn oCLSendMove;
	extern SetupBonesFn SetupBones;
	...
	//extern get_eye_ang::fn GetEyeAng;

	void Hook();
	void UnHook();
}

extern void PNotify(string Owner, string Message, int icon);
